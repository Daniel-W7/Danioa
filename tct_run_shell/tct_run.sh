...
case $1 in
	-sw|--software)
		...
		sed -i s/^INSTALL_OPTION=.*/INSTALL_OPTION=software/g /usr/local/tctconfig/bin/{tctconfig,removtct}
	...
	-sc|--script)
		...
		echo PATH='$PATH':$HOME/tctconfig/bin >> $HOME/.bashrc
                ...
                rm -rf $HOME/tctconfig/package/update/*
                sed -i s/^INSTALL_OPTION=.*/INSTALL_OPTION=script/g $HOME/tctconfig/bin/{tctconfig,removtct}
